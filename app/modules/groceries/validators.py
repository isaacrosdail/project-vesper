...
def validate_net_weight(net_weight: str | None) -> tuple[float | None, list[str]]:
    ...
    is_valid, error_type = v.validate_numeric(net_weight,
                        c.NET_WEIGHT_PRECISION,
                        c.NET_WEIGHT_SCALE,
                        c.NET_WEIGHT_MINIMUM)
    if not is_valid:
...
def validate_calories(calories: str | None) -> tuple[float | None, list[str]]:
    ...
    if calories:
        is_valid, error_type = v.validate_numeric(calories,
                                             c.CALORIES_PRECISION,
                                             c.CALORIES_SCALE,
                                             c.CALORIES_MINIMUM)
        if not is_valid:
            if error_type in [v.FORMAT_ERROR, v.PRECISION_EXCEEDED, v.SCALE_EXCEEDED]:
