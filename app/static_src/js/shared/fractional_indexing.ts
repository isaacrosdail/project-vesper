// TODO: Study; for task list view reordering
...
function midpoint(a, b, digits) {
  ...
  // Upper bound given, we're computing a key strictly between a and b
  // Common-prefix-stripping trick only makes sense when we have two endpoints to compare
  if (b) {
    // Remove longest common prefix. pad 'a' with 0s as we go.
    // Note that we don't need to pad 'b' because it can't end before 'a' while traversing the common prefix.
    let n = 0;
    // (a[n] || zero) -> virtual infinite padding on a? In JS, a[n] rets undefined when
    // n is past the end of the string. undefined || "0" -> "0"
    // On each loop iter, when we access a[n] and it's undefined (past the end),
    // the '|| zero' swaps in "0" for that single comparison?
    // "pretend-zeros on demand"
    while ((a[n] || zero) === b[n]) {
      n++;
    }
    if (n > 0) {
