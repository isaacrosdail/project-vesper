...
class APISerializable:
    ...
    def to_api_dict(self) -> dict[str, Any]:
        ...
        # Mapper is a Mapper object
        # SQLAlchemy's internal representation of how
        # our Python class Task maps to a DB table
        mapper = inspect(self.__class__) # self = Task instance
        # mapper.columns is a collection of Column objects
        # -> not the values, the column definitions themselves

        # Configuring excludes based on both base exclude list here plus
        # whatever the model itself dictates
        ...
        for col in mapper.columns: # type: ignore[union-attr]
            # 'col.name' = name of the field/column (eg, 'priority', 'due_date')
            if col.name in exclude:
                continue # skip excluded fields

            # 'value' here is the actual data in this instance
            # Represents the actual values stored in this Task instance
            # )(eg, 'PriorityEnum.LOW', 'datetime(2025, 10, 7)', '7', etc)
