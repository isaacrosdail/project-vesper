...
def test_tasks_dashboard_displays_tasks(client, db_session):
...
# Test POST
def test_add_task_creates_task(client, db_session):
...
def test_complete_task(client, db_session, sample_task):
    ...
    task = db_session.query(Task).filter_by(title="Complete Me").first()
