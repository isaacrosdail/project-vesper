...
from app.modules.groceries.models import (Product, ProductCategoryEnum,
                                          Transaction, UnitEnum)
...
def test_task_roundtrip():
    ...
    assert result["due_date"] == "2025-10-08T14:00:00+00:00"  # Datetime converted to ISO
...
def test_dailyentry_serialization():
    entry = DailyEntry(
        weight=175.5,
        steps=8500,
        calories=2100,
        wake_time=datetime(2025, 10, 7, 14, 0, tzinfo=timezone.utc),
        sleep_time=datetime(2025, 10, 7, 3, 0, tzinfo=timezone.utc)
    )
    ...
    assert result["wake_time"] == "2025-10-07T14:00:00+00:00"
    assert result["sleep_time"] == "2025-10-07T03:00:00+00:00"
    assert result["subtype"] == "daily_entries"
    assert "user_id" not in result
