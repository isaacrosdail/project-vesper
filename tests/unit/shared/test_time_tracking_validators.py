...
def test_validate_duration(duration, expected):
    assert validate_duration(duration) == expected


@pytest.mark.parametrize("data, expected", [
    ({"category": "Study", "description": "Algorithms", "duration": "45"}, {})
])
...
@pytest.mark.parametrize("data, expected", [
    ({"category": "", "description": "a" * 201, "duration": "-5"}, 
    {
        "category": [CATEGORY_REQUIRED],
        "description": [DESCRIPTION_LENGTH],
        "duration": [DURATION_POSITIVE]
    })
])
