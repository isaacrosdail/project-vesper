...
def test_unit_type_invalid():
    result = validate_unit_type("not_a_unit")
    # Check that it contains "Invalid" and the valid options
    assert any("Invalid" in err for err in result)
    assert any(unit.value in result[0] for unit in UnitEnum)
