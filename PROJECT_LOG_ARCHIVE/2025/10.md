...
## [Thurs 2.10.25]
**Log:**
- Add validate_time_hhmm() in shared validators, add test cases
...
- Scrapped `parse_eod_datetime_from_date` helper - replaced with validate_date_iso in shared validators to parse raw date_str + to_eod_datetime in service layer for tasks to convert that date into a full EOD datetime
...
## [Sat 4.10.25]
**Log:**
- Viewmodels + TimestampedMixin Tuning
...
- Retooling "Add Transaction" form:
...
## [Sun 5.10.25]
**Log:**
- Refactored `add_transaction` flow (product-driven instead of barcode)
    - Route logic:
...
- CRUD Update flow (WIP, started with Tasks)
...
## [Tues 7.10.25]
**Log:**
- `APISerializable` Implementation
...
- Submission/Validator fix (updated tests accordingly)
...
- Habits Progress Bar (initial draft)
...
## [Fri 10.10.25]
**Log:**
- Add proper SVG icon for dots option button on side of tables
- Habits Progress Bar Polish
...
## [Sat 11.10.25]
**Log:**
- Make similar Task progress bar for current day
- Expand habit progress logic to per-habit as well
...
## [Thurs 23.10.25]
**Goals:**
- Improve time_tracking entry form/service to take dates+times
**Log:**
- Improve time_tracking entry form/service to take dates+times
...
## [Mon 27.10.25]
**Log:**
- D3 Charting - add line chart to metrics
- Metrics service - update/fix service stuff, include sleep duration & entry_datetime changes, also changed repo stuff a bit
...
## [Wed 29.10.25]
**Log:**
- Fixed detached tooltip carrot: just made the tooltip-popup itself positioned centered to targetEl
- Hooked up habits barchart to actual data, made backend route, tweaked repo funcs
