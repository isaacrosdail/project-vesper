...
**Next Time:**
- Tweak `Groceries` dashboard to support delete-on-hover behavior
...
- Add script to enable npm run dev to run Flask app, BrowserSync, and Tailwind watch
...
- Installed `cross-env` & set up npm run build script for Tailwind:
...
## [Sun 11.05.25]
**Goal:**
- Auto-seed dummy data in dev mode after app startup
**Log:**
...
**Goal:**
- Add "Reset" button for dummy DB data (drop tables → run `seed_data.py` again)
**Log:**
...
**Next Up:**
- Add logic to redirect `reset_db` route back to referrer page (referrer check)
...
- Write `README.md` (features, setup)
- Refactor code structure / architecture
- Add comments to complex logic
- Prune comments in main + merged branches
...
**Next Up:**
- Clean up readme/tech stack info, w/ decisions for choices
- 2. Clean up readme/tech stack info (with decisions for choices)
- Sort out dev/prod database separation (dev-> personal use, prod-> demo info only)
	Options:
...
- Add "This Week's Improvements" feature
...
## [Sun 25.05.25]
**Goals:**
- Not much today, mostly worked on resume site
