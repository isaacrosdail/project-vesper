...
**Next Up:**
- Fix input handling so user can type `stop` during countdown
...
**Next time:**
- Tweak `add_product` to detect repeats and increment quantity  
- Fix `grocery.html` to generate table headers dynamically


## [Tue 25.03.25]

**Goals:**
- Tweak `add_product` logic to detect repeat scans and increase quantity
- Make `grocery.html` table headers generate dynamically
**Log:**
...
**Next Time:**
- Implement `add_product` route: accept scanned barcode, enter rest of product info, add to DB, show in UI  
- Set up Flask-Migrate: init, create/apply schema version, test with `Product` model
