...
- Tables, Toast Styling, Mobile Nav
...
## [Tues 02.09.25]
**Log:**
- Finally integrated / sifted through our backlog of day logs!
- Tackling loose ends
...
- Revamping database/operations.py
...
- Improving some of our older JS
...
## [Fri 05.09.25]
**Log:**
- Divied up PROJECT_LOG into archival files, separated by year/month

## [Sun 07.09.25]
**Log:**
- Shopping List feature: Started small but extensible:
...
- Added api.js to clean up that disgusting fetch boilerplate all over & centralize response formats
...
## [Wed 10.09.25]
**Log:**
- Added last few days to project log
- Quick UI Fixes
...
- Standardize module/subtype naming conventions a bit, added new CONVENTIONS.md for documenting these centrally
- CSS tidying
...
**Log:**
- CSS for buttons: Retooled to better accommodate text buttons vs icon (SVG) buttons
...
## [Tues 16.09.25]
**Log:**
- Stepped down all neutrals in light mode facilitate stepping UP for hover/active states

## [Wed 17.09.25] Dark Mode & Color System Tuning
**Log:**
- CSS: Completed dark mode color tuning & OKLCH conversion
...
**Log:**
- Implemented `markTaskComplete` for tasks list on homepage as well
- Tabbed modal (metric_entry) work:
...
- Drafted hamburger menu -> X transition effect too
...
**Log:**
- CSS: practice with `::before`/`::after` & transitions
	- Replace bg-color nav-link effect with expanding underline using `::before`/`::after`
- Added basic validation + test cases to get us mostly functional
...
- Started integrating `validators.py` changes into routes/service layers
...
**Log:**
- Continued organization/trimming of style-reference Page
...
**Log:**
- Rebased to squash/edit quite a few commits
- Model standardization & Enum overhaul
...
**Log:**
- Explored Hypothesis (property-based testing); tweaked parse_decimal to handle Inf, added `.coveragerc` to exclude certain paths from pytext-cov
- Models refactoring:
...
## [Sun 28.09.25]
**Log:**
- Dissolve `core/` -> Move templates to _templates, healthcheck to app/routes.py
- Smoke Testing Forms: tasks (frog + normal), habits (promotion-based + normal)
- Tweaking
...
## [Mon 29.09.25]
**Log:**
- Uppercase unit_enum values (tweak models.py, revision with manual op.executes since Postgres is touchy about this). Updated viewmodel to include .lower()
- Smoke testing forms

## [Tues 30.09.25]
**Log:**
- Install time-machine to start testing datetime-related functions
- Testing datetimes
	- view_mixins: Test for due_date_label
	- Generally start testing other datetime helpers
- Datetime helpers changes/improvements
