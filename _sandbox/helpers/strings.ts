...
// Problem: This just lazily finds the FIRST `left`, meaning we won't reliably match what we want, but rather "whatever looks similar and occurs first"
// We fixed this stuff in getCookie() in theme-manager.ts using split() + find()
export function between(string: string, left: string, right: string): string {
    const l = string.indexOf(left) + left.length;
    if (l === -1) return ""; // what was the issue here? Hint: when would this check happen?
...
////// Helpful loop for rapid testing on prototyping
// while (true) {
//   const raw = prompt("Enter a value (or 'q' to quit): ");
//   if (raw === null || raw.trim().toLowerCase() === 'q') break;

//   const [a, b] = raw.split(',')
//   const bStr = Number(b)
//   const result = formatDecimal(a, bStr);
//   console.log(`${result}`);
// }